...
pub fn get_default_recordings_folder() -> PathBuf {
    ...
    {
        // Windows: %USERPROFILE%\Music\meetfree-recordings
        if let Some(music_dir) = dirs::audio_dir() {
            music_dir.join(crate::brand::RECORDINGS_DIR_NAME)
        } else {
            // Fallback to Documents if Music folder is not available
            dirs::document_dir()
                .unwrap_or_else(|| PathBuf::from("."))
                .join(crate::brand::RECORDINGS_DIR_NAME)
        }
    }

    #[cfg(target_os = "macos")]
    {
        // macOS: ~/Movies/meetfree-recordings
        if let Some(movies_dir) = dirs::video_dir() {
            movies_dir.join(crate::brand::RECORDINGS_DIR_NAME)
        } else {
    ...
    {
        // Linux/Others: ~/Documents/meetfree-recordings
        dirs::document_dir()
            .unwrap_or_else(|| PathBuf::from("."))
            .join(crate::brand::RECORDINGS_DIR_NAME)
    }
