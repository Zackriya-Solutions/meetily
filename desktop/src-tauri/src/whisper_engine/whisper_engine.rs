...
impl WhisperEngine {
    ...
    pub fn new_with_models_dir(models_dir: Option<PathBuf>) -> Result<Self> {
        let models_dir = if let Some(dir) = models_dir {
            ...
            // Development: use repository-local models directories when present.
            // Production: Use system directories (should be overridden by caller)
            if cfg!(debug_assertions) {
                // Development mode - try models directories near the current working directory.
                if current_dir.join("models").exists() {
                    current_dir.join("models")
                } else if current_dir.join("../models").exists() {
                    current_dir.join("../models")
                } else {
                    // Create a local models directory during development.
                    current_dir.join("models")
                }
            } else {
