...
function DownloadToastContent({
  ...
  return (
    <div className="flex items-center gap-3 w-full max-w-sm bg-white dark:bg-[#1e2028] rounded-lg shadow-lg border border-gray-200 dark:border-white/10 p-3 relative">

      {/* Icon */}
      <div className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center ${isComplete ? 'bg-green-100 dark:bg-green-900/30' : hasError ? 'bg-red-100 dark:bg-red-900/30' : isCancelled ? 'bg-gray-100 dark:bg-white/10' : 'bg-gray-100 dark:bg-white/10'
        }`}>
        {isComplete ? (
          ...
          <ArrowBigDownDash className="size-5 text-gray-600 dark:text-gray-300" />
        )}
      </div>
      ...
      <div className="flex-1 min-w-0">
        ...
        {hasError ? (
          ...
          <p className="text-xs text-gray-600 dark:text-gray-400">Download cancelled</p>
        ) : (
          <>
            {/* Progress bar */}
            <div className="w-full h-1.5 bg-gray-200 dark:bg-white/10 rounded-full overflow-hidden mb-1.5">
              <div
