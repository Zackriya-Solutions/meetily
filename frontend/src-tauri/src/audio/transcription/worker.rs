...
pub fn start_transcription_task<R: Runtime>(
    ...
    tokio::spawn(async move {
        ...
        // Initialize transcription engine (Cohere ONNX — single provider)
        ...
        for worker_id in 0..NUM_WORKERS {
            let engine_clone = match &transcription_engine {
                TranscriptionEngine::CohereOnnx(e) => TranscriptionEngine::CohereOnnx(e.clone()),
            };
                        ...
                        Some(chunk) => {
                            ...
                            .await
                            {
                                Ok((transcript, confidence_opt, is_partial)) => {
                                    // Cohere provider returns no confidence score — accept everything.
                                    let confidence_threshold = match &engine_clone {
                                        TranscriptionEngine::CohereOnnx(_) => 0.0,
                                    };
...
/// Transcribe a single audio chunk through the Cohere ONNX provider.
...
async fn transcribe_chunk_with_provider<R: Runtime>(
    ...
    match engine {
        TranscriptionEngine::CohereOnnx(cohere_engine) => {
            let language = crate::get_language_preference_internal();
            let provider =
                super::cohere_provider::CohereProvider::new(cohere_engine.clone());

            match super::TranscriptionProvider::transcribe(&provider, speech_samples, language)
                .await
            {
                Ok(result) => {
                    ...
                    info!(
                        "Cohere transcription complete for chunk {}: '{}' (partial: {})",
                        chunk.chunk_id, cleaned_text, result.is_partial
                    );
                ...
                Err(e) => {
                    error!(
                        "Cohere transcription failed for chunk {}: {}",
                        chunk.chunk_id, e
                    );
