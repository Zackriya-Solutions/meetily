...
pub async fn start_import<R: Runtime>(
    ...
    // Provider selection is no longer meaningful — Cohere ONNX is the single
    // local transcription path. `provider` is still accepted for backward
    // compatibility with clients that pass it.
    let _ = provider;
...
async fn run_import<R: Runtime>(
    ...
    // Initialize the Cohere ONNX engine once if there is anything to transcribe.
    let cohere_engine = if total_segments > 0 {
        Some(get_or_init_cohere(&app, model.as_deref()).await?)
    } else {
        None
    };
    ...
    for (i, segment) in processable_segments.iter().enumerate() {
        ...
        // Transcribe this segment through Cohere ONNX.
        let engine = cohere_engine
            .as_ref()
            .ok_or_else(|| anyhow!("Cohere engine missing while processing segments"))?;
        let text = engine
            .transcribe_audio(segment.samples.clone(), 16_000, language.clone())
            .await
            .map_err(|e| anyhow!("Cohere transcription failed on segment {}: {}", i, e))?;
        // Cohere does not expose a per-segment confidence score.
        let conf = 0.9f32;
...
/// Get or initialize the Cohere ONNX engine, loading the requested (or configured) model if needed.
...
/// Resolve the configured Cohere model name from the database, falling back to DEFAULT_COHERE_MODEL.
async fn get_configured_cohere_model<R: Runtime>(app: &AppHandle<R>) -> Result<String> {
    ...
    Ok(match result {
        Some((provider, model)) if provider == "cohere" && !model.is_empty() => model,
